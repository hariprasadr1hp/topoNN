...
from pythonlib.solvefe_2d import SolveFE2D
...
class DataGen:
    ...
    def __init__(self, mesh: Plate2D, amount: int) -> None:
        self.mesh = mesh
        self.nelx = mesh.nelx
        self.nely = mesh.nely
        self.amount = amount
    ...
    def generate_forces(self, hdf_file):
        """
        ...
        with h5py.File(self.fname, "w") as f:
            ...
            boundary_condns = util.formulate_2d_condns(self.mesh.get_down(), (-1, -1))
            for i in range(1000):
                force_condns = util.formulate_2d_condns(
                    self.mesh.get_up(), (rand_vals_100(), rand_vals_100())
                )
                strain_density = SolveFE2D(self.mesh, boundary_condns, force_condns)
                g1.create_dataset("config_{i}", data=strain_density)
            #########################################################
            # CASE 2: constrained: TOP, FORCE: BOTTOM
            #########################################################
            g2 = f.create_group("T2B1L0R0")
            boundary_condns = util.formulate_2d_condns(self.mesh.get_up(), (-1, -1))
            for i in range(1000):
                force_condns = util.formulate_2d_condns(
            ...
            boundary_condns = util.formulate_2d_condns(self.mesh.get_left(), (-1, -1))
            for i in range(1000):
                force_condns = util.formulate_2d_condns(
            ...
            boundary_condns = util.formulate_2d_condns(self.mesh.get_right(), (-1, -1))
            for i in range(1000):
                force_condns = util.formulate_2d_condns(
